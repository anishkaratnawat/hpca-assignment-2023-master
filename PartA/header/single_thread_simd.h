...
void singleThread(int input_row, int input_col, int *input, int kernel_row,
  ...
  for (int kernel_i = 0; kernel_i < kernel_row; kernel_i++) {   // C
    for (int output_i = 0; output_i < output_row; output_i++) { // A
      int input_i = (output_i + 2 * kernel_i) % input_row;
      for (int kernel_j = 0; kernel_j < kernel_col; kernel_j += 4) {   // D
        for (int output_j = 0; output_j < output_col; output_j += 2) { // B
          int input_j = (output_j + 2 * kernel_j) % input_col;
          __m256i input_vec;
          ...
          int k3 = (kernel_j + 3 < kernel_col)
                       ? kernel[kernel_i * kernel_col + kernel_j + 3]
                       : 0;
          int k2 = (kernel_j + 2 < kernel_col)
                       ? kernel[kernel_i * kernel_col + kernel_j + 2]
                       : 0;
          int k1 = (kernel_j + 1 < kernel_col)
                       ? kernel[kernel_i * kernel_col + kernel_j + 1]
                       : 0;
          int k0 = (kernel_j < kernel_col)
                       ? kernel[kernel_i * kernel_col + kernel_j]
                       : 0;
          ...
          __m128i sum128 =
              _mm_add_epi32(_mm256_castsi256_si128(output_vec),
                            _mm256_extracti128_si256(output_vec, 1));
          auto o1 = _mm_extract_epi32(sum128, 0) + _mm_extract_epi32(sum128, 1);
          auto o2 = _mm_extract_epi32(sum128, 2) + _mm_extract_epi32(sum128, 3);
          output[output_i * output_col + output_j] += o1;
          output[output_i * output_col + output_j + 1] += o2;
        }
      }
    }
  }
